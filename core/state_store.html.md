...
The `WorkflowStateStore` protocol defines the interface for persisting
workflow state. This allows patterns like `StepFlow` to work with
different storage backends (in-memory, file-based, database) without
...
*Set the active workflow session ID, switching all subsequent state
reads/writes to that session.*
...
A simple in-memory implementation of `WorkflowStateStore` suitable for
development and testing. State is lost when the server restarts.
