...
The `Step` class defines a single step in a multi-step workflow. Each
step has: - Unique identifier - Title for display - Render function that
generates the UI - Optional validation function - Optional data loader
for fetching required data - State keys that this step manages
...
The `StepFlow` class manages a multi-step workflow. It: - Tracks current
step - Manages navigation between steps - Uses a `WorkflowStateStore`
...
The `StepFlow` class uses `InMemoryWorkflowStateStore` by default, so
you don’t need to provide a state store for simple use cases.
