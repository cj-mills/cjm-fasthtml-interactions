...
The `Pagination` class manages paginated views with automatic route
generation and state management. It follows the same pattern as
`StepFlow` for consistent API design.
...
The `Pagination` class integrates seamlessly with FastHTML applications
using the same pattern as `StepFlow`.
...
After creating a `Pagination` instance and generating its router,
register it with your FastHTML app:
...
The `Pagination` class automatically configures HTMX attributes for
SPA-like navigation:
