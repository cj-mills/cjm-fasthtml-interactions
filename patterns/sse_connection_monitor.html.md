...
The `SSEConnectionConfig` dataclass defines configuration options for
the SSE connection monitor.
...
The `SSEConnectionMonitorScript` function creates a JavaScript script
that monitors SSE connection status.
...
The `SSEConnectionMonitor` function creates a complete connection
monitoring system with status indicator and monitoring script.
